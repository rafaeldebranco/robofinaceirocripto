...
*   **Histórico de Operações:** Consulta o histórico de trades do usuário para determinar a última compra.
*   **Estratégia de Lucro (1%):** Implementa uma rotina de compra e venda automática com uma estratégia simples de lucro de 1% (parâmetro configurável). Se não houver posição, compra. Se houver, calcula o preço de venda com 1% de lucro e executa a venda se o preço for atingido.
...
O robô agora inclui uma função `trading_strategy()` que implementa a lógica de compra e venda automática. Para evitar execuções acidentais com chaves de API não configuradas, a estratégia é executada apenas uma vez no bloco `if __name__ == "__main__":`. Para uso contínuo, o loop `while True` deve ser descomentado.

### Funções Principais:

*   `trading_strategy(instrument_name, quantity, profit_percentage)`: Executa a lógica de compra e venda automática.
...
*   **Estratégias de Trading:** A estratégia de lucro de 1% é um ponto de partida. Implemente lógicas mais avançadas para análise de mercado, indicadores técnicos e tomada de decisão.
